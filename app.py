...
def text_analysis(raw_text, num_of_most_common=5):
    ...
    with col1:
        ...
        with st.expander("Top Keywords"):
            ...
            keywords = get_most_common_tokens(processed_text, num=num_of_most_common)
            st.write(keywords)
    ...
    if choice == "📚NLP":
        ...
        if image_file is not None:
            with st.expander("File Stats"):
                ...
                df_file_details_combined = pd.DataFrame(
                    file_details_combined.items(), columns=["Meta Tag", "Value"]
                ).astype(str)
            ...
            with col2:
                with st.expander("Default(JPEG)"):
                    ...
                    df_img_details = pd.DataFrame(
                        img_details.items(), columns=["Meta Tag", "Value"]
                    ).astype(str)
            ...
            with fcol1:
                with st.expander("Exifread Tool"):
                    ...
                    df_meta_tags = pd.DataFrame(
                        meta_tags.items(), columns=["Meta Tag", "Value"]
                    ).astype(str)
            ...
            with st.expander("Download Results"):
                final_df = pd.concat(
                    [df_file_details_combined, df_img_details, df_meta_tags]
                )
        ...
        if audio_file is not None:
            ...
            with st.expander("Metadata with Mutagen"):
                ...
                df_audio_details = pd.DataFrame(
                    file_details.items(), columns=["Meta Tag", "Value"]
                ).astype(str)
            ...
            with col2:
                with st.expander("Metadata with Mutagen"):
                    ...
                    df_pdf_info = pd.DataFrame(
                        pdf_info.items(), columns=["Meta Tag", "Value"]
                    ).astype(str)
        ...
        st.subheader("Files Analytics")
        all_upload_files = view_all_data()
        df = pd.DataFrame(
            all_upload_files, columns=["filename", "filetype", "filesize", "uploadDate"]
        )
        ...
        with st.expander("Distribution of files"):
            fig = plt.figure()
